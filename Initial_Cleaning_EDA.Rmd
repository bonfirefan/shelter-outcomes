...
```{r}
breed_reference <- as.data.frame(unique(cad$breed_new))
names(breed_reference) <- "breed_new"
breed_reference <- left_join(breed_reference, 
                             breed_groups, by = c("breed_new" = "breeds"))
write.csv(breed_reference, 'breed_reference.csv')

breed_groups <- read.csv("breedGroups.csv")
color_groups <- read.csv("colorGroups.csv")

cad <- left_join(cad, breed_groups, by = c("breed_new" = "breeds"))
cad <- left_join(cad, color_groups, by = c("Color" = "colors"))
```
...
```{r fig.width=10, fig.height=10}
g <- ggplot(data=plot_data, aes(x=Breed, fill= OutcomeType )) 
g + geom_bar(stat="count", position=position_dodge()) +
    xlab("Breed") +
    ggtitle("Distribution of breed on different OutcomeType") +
...
g + geom_bar(stat="count", position=position_dodge()) +
    xlab("Animal type") +
    ggtitle("Distribution of animal type on different OutcomeType") +
    facet_grid(~ OutcomeType)
```
...
g + geom_bar(stat="count", position=position_dodge()) +
    xlab("Whether an animal has a name") +
    ggtitle("Distribution of whether an animal has a name on different OutcomeType") +
    facet_grid(~ OutcomeType)
```
...
g + geom_bar(stat="count", position=position_dodge()) +
    xlab("Sexual organ status") +
    ggtitle("Distribution of Sexual organ status on different OutcomeType") +
    facet_grid(~ OutcomeType)
```
```{r}
...
g + geom_bar(stat="bin", position=position_dodge()) +
    ...
    ggtitle("Distribution of AgeuponOutcom on different OutcomeType") +
    facet_grid(~ OutcomeType)
```

#Save off data frame for use at later points
```{r}
write.csv(cad, file="Data/Cleaned_Data_Frame.csv")
```
