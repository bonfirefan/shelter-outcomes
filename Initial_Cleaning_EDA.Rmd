...
- The outcome by animal type was not overly infrrmative like we hoped it might be. For the most part this seems to have to do with the fact that the dataset is heavily skewed towards dogs vs cats. ~2/3s of the data set is dogs, thus outcomes by animal type doesn't tell us as much information as we first expected.

The most important decision that came out of our data cleaning and transformation process has to do with our decision not to include any outcome variables that were provided in the original dataset (other than animal's fixed status**), due to the fact that we felt this would be unrealistic to include in our modeling portion of the project. The reasoning being that since the animal shelter would look to use our models to help predict animal outcomes and provide resources to increase an animals chance for a positive outcome, they would not know an animal's outcome information in advance. Thus moving forward into our modeling portion of the project, we foucs our attention on data that the animal shelter would be able to assess from an animal upon intake to the shelter. 

**The reason we decided to keep the animal's fixed status has to do with the fact that a shelter can leverage it's resources to perform these procedures on their shelter animals and knowing what fixed statuses lead to better outcomes would help them better decide which animals might benefiit from surgical intervention.
