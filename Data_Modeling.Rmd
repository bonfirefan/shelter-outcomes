...
confusionMatrix(data=svmModel, testing.set$OutcomeType)
```
...
rfConfusion <- confusionMatrix(data=rf_pred, testing.set$OutcomeType)
rfConfusion
```
...
Now that we have found the optimized parameters, we run a slightly more targetted final Random Forest model.
...
Save off the final fit for the Random Forest model.
